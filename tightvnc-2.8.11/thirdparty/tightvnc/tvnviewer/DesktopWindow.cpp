  ...
  m_isBackgroundDirty(false)
{
	...
	m_rectLimit.left = 10;
	m_rectLimit.top = 10;
	m_rectLimit.right = 50;
	m_rectLimit.bottom = 50;
	m_rectDrawText.left = 10 + 50;
	m_rectDrawText.top = 10;
	m_rectDrawText.right = 50 + 100;
	m_rectDrawText.bottom = 50;	
}
...
bool DesktopWindow::onMouse(unsigned char mouseButtons, unsigned short wheelSpeed, POINT position)
{
  ...
  m_ptCurrentMousePos.x =  pos.x;
  m_ptCurrentMousePos.y =  pos.y;
  if (PtInRect(&m_rectLimit, m_ptCurrentMousePos))
  {
	  m_logWriter->detail(_T("Mouse pos: %d, %d"),  pos.x, pos.y);
  }
...
void DesktopWindow::doDraw(DeviceContext *dc)
{
  ...
  if (PtInRect(&m_rectLimit, m_ptCurrentMousePos))
  {
	  drawText(dc, TEXT("hello,world"), &m_rectDrawText);
  }
}
...
void DesktopWindow::drawText(DeviceContext *dc, const TCHAR *text, RECT *rect)
{
	Graphics graphics(dc);
	graphics.setBkMode(true);
	graphics.drawText(text, -1, rect, DT_CENTER|DT_VCENTER);
}
