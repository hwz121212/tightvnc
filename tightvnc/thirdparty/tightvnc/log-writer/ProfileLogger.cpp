...
std::vector<TCHAR> printVectorStats(const TCHAR *header, const TCHAR *tag1, const TCHAR *tag2, std::vector<double> v) {
  ...
  const TCHAR *fmt = _T("%s for %s - %s distance: avg: %f, min: %f, max: %f, executed %d times\n");
...
std::vector<std::vector<TCHAR>> ProfileLogger::dropStat()
{
  ...
  for (d = deltas.begin(); d != deltas.end(); ++d) {
    ...
    std::vector<TCHAR> str = printVectorStats(_T("CPU cycles"), tag1, tag2, (*d).second.deltac);
    resultStrings.push_back(str);
    str = printVectorStats(_T("Process times"), tag1, tag2, (*d).second.deltap);
    resultStrings.push_back(str);
    str = printVectorStats(_T("Kernel times"), tag1, tag2, (*d).second.deltak);
    resultStrings.push_back(str);
  }
  m_checkPoints.clear();
  return resultStrings;
